---
title: "CRJ504 - Inference for Categorical Data"
...
* Datasets for homework assignments have been updated. Located here: https://github.com/jbryer/CRJ504Fall2018/tree/master/data/os3_data
...
```{r}
gss2010 <- read.csv('../data/os3_data/Ch 5 Exercise Data/gss2010.csv')
gss2010$degree <- factor(as.character(gss2010$degree), 
						 levels = c('LT HIGH SCHOOL', 'HIGH SCHOOL', 'JUNIOR COLLEGE',
						 		   'BACHELOR', 'GRADUATE'), 
						 ordered = TRUE)
tab <- psych::describeBy(gss2010$hrs1, group = gss2010$degree, mat = TRUE)[,c('group1','mean','sd','n')]
tab
```
...
```{r, warning=FALSE}
ggplot(gss2010, aes(x = degree, y = hrs1)) + geom_boxplot() + 
...
$$ H_{0}: \mu_{LTHS} = \mu_{HS} = \mu_{JC} = \mu_{B} = \mu_{G} $$

## Exercise 5.48 (cont.) {.columns-2}
...
ggplot(gss2010, aes(x=hrs1, fill=degree, 
					color=degree)) + 
...
```{r}
summary(aov(hrs1 ~ degree, data = gss2010))
```
